...
public class CommonUtils {
    ...
    public static <K, V> void listGroup2Map(List<V> list, Map<K, List<V>> map, Class<V> clazz, String methodName) {
        ...
        if (null == list || null == map || null == clazz) {
            System.out.print("CommonUtils.listGroup2Map 入参错误，list：" + list + " ；map：" + map + " ；clazzs：" + clazz + " ；methodName：" + methodName);
            return;
        }
    ...
    public static Method getMethodByName(Class<?> clazz, String methodName) {
        ...
        // 入参不能为空
        if (null == clazz) {
            System.out.print("CommonUtils.getMethodByName 入参错误，clazzs：" + clazz + " ；methodName：" + methodName);
            return method;
        }
